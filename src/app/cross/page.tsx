		...
		<div className="min-h-[100dvh] gradient-mesh">
			<div className="mx-auto w-full max-w-2xl px-5 pb-28 lg:pb-10">
				...
				{!entry && !error ? (
					...
					<>
						<div className="glass-card gradient-border flex flex-col gap-3 rounded-2xl p-6">
							<p className="text-[15px] font-bold text-amber-600 dark:text-amber-400">{entry.reference}</p>
						...
						{entry.whyToday && (
							<section className="mt-6 flex flex-col gap-2">
								<h2 className="text-[11.5px] font-bold tracking-[0.1em] text-amber-700/70 dark:text-amber-500/60">
							...
							<section className="mt-6 flex flex-col gap-2">
								...
								{entry.studyPath.map((step, index) => {
									...
									return href ? (
										...
										<div
											key={`${step.book}-${step.chapter}-${index}`}
											className="flex flex-col gap-1 rounded-xl border border-black/[0.08] dark:border-white/[0.06] bg-black/[0.03] dark:bg-white/[0.03] px-4 py-3"
										>
											{body}
										</div>
									);
						...
						{entry.question && (
							<div className="mt-6 flex flex-col gap-2 rounded-2xl border border-amber-500/40 dark:border-amber-400/30 bg-amber-500/10 dark:bg-amber-400/10 p-5">
								<h2 className="text-[11.5px] font-bold tracking-[0.1em] text-amber-700/70 dark:text-amber-500/60">
									CARRY THIS
								</h2>
						...
						<button
							type="button"
							onClick={goDeeper}
							className="mt-6 flex min-h-12 w-full items-center justify-center rounded-xl border border-amber-500/40 dark:border-amber-400/30 bg-amber-500/10 dark:bg-amber-400/10 text-[15px] font-bold text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 dark:hover:bg-amber-400/20 transition-colors"
						>
							✦ Go deeper in chat
						</button>
					</>
