...
const NoteCard: React.FC<NoteCardProps> = ({ note, tags, isActive, onClick }) => {
	...
	return (
		<button
			onClick={onClick}
			className={`
				w-full h-full flex flex-col text-left p-4 rounded-xl border transition-all duration-150 animate-message-in
				${isActive
		...
		>
			<div className="flex items-start justify-between gap-2 mb-1">
				<h3 className="text-neutral-800 dark:text-neutral-200 font-medium text-sm truncate flex-1">
			...
			<p className="text-neutral-500 text-xs line-clamp-2 mb-2 flex-1">{snippet}</p>
