 ...
 *
 * Mirrors `mobile/src/features/plan/planView.ts` exactly (that copy carries
 * the vitest suite), which in turn mirrors `describeReadings` in
 * `src/lib/reading-plan-presets.ts` on the server.
 */
...
/**
 * The one-line summary for the Bible page's plan card: where they are, or an
