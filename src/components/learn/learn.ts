/**
 * N4 Learn a verse: the client half of the contract in docs/FEATURES.md.
 * Mirrored in mobile/src/features/learn/learn.ts — keep identical. Pure
 * functions only, so tests/learn-mask.test.mjs can drive it with plain node.
 */
...
/**
 * The masking ladder. Stage 0: read the whole verse. Stage 1: hide every
 * word whose index % 4 === 3. Stage 2: hide index % 2 === 1. Stage 3: every
 * word hidden, only the reference shows. Whitespace defines a word; edge
 * punctuation stays visible next to the blank, so "world:" shows "____:".
 */
...
export function maskVerse(text: string, stage: LearnStage): string {
	return verseWords(text, stage)
		.map((word) => (word.hidden ? word.blank : word.text))
		.join(" ");
}
...
export function parseCard(value: unknown): LearnCard {
	...
	if (
		...
		!card.id ||
		!Number.isInteger(card.book) ||
		card.book! < 1 ||
		card.book! > 66 ||
		!Number.isInteger(card.chapter) ||
		card.chapter! < 1 ||
		!Number.isInteger(card.verse) ||
		card.verse! < 1 ||
		...
		!Number.isInteger(card.intervalDays) ||
		card.intervalDays! < 0 ||
		typeof card.dueAt !== "string" ||
		!Number.isFinite(Date.parse(card.dueAt)) ||
		!(
			card.knownAt === null ||
			(typeof card.knownAt === "string" && Number.isFinite(Date.parse(card.knownAt)))
		)
	) {
...
export function parseToday(value: unknown): LearnToday {
	...
	if (
		!today ||
		!Array.isArray(today.cards) ||
		today.cards.length > 3 ||
		!Number.isInteger(today.knownCount) ||
		today.knownCount! < 0 ||
		!Number.isInteger(today.queueCount) ||
		today.queueCount! < 0
	) {
...
export function applyReview(
	...
	return {
		...
		cards: finished
			...
			: today.cards.map((card) => (card.id === before.id ? updated : card)),
	};
}
