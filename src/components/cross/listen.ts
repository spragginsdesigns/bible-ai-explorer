...
export interface DailyCrossAudio {
	 ...
	 * Kept because it fetches fine, but never given to a media element - Chrome
	 * will not load it. Play from `streamUrl`.
	 */
	url: string | null;
	/**
	 * Same-origin path that proxies the narration; only present while status is
	 * "ready". See `src/app/api/verse-of-day/audio/stream/route.ts`.
	 */
