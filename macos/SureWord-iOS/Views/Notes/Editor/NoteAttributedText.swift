...
/// Bridges `NoteDocument` and the `NSAttributedString` a `UITextView` edits —
/// the iOS counterpart of the macOS `NoteAttributedText`, with the same
/// private attribute keys and the same one-to-one character mapping so the
/// shared round-trip contract holds on both platforms.
///
...
/// `.noteMarks` per run. Everything UIKit renders — fonts, colours, indents —
/// is derived from those, never read back out of them. That direction matters:
/// inferring "bold" from a `UIFont` trait would lose the distinction between a
...
enum NoteAttributedText {
    ...
    /// Attribute values must be objects, and the text system coalesces adjacent
    /// runs by `isEqual:` — so these boxes need real equality, not identity, or
    /// every character would start its own run.
    ...
    private static func apply(
        ...
        for mark in marks {
            ...
            case .code:
                isCode = true
                attributes[.backgroundColor] = UIColor(theme.surfaceStrong)
            case .highlight:
                attributes[.backgroundColor] = UIColor(theme.accent.opacity(0.25))
            case .link:
    ...
    private static func font(_ base: UIFont, adding traits: UIFontDescriptor.SymbolicTraits) -> UIFont {
        guard
