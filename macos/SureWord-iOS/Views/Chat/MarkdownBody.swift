...
/// Port of `macos/SureWord/Chat/Views/MarkdownBody.swift` (itself a port of
/// `mobile/src/features/chat/MarkdownBody.tsx`) — same block styles, same
/// signature look: quoted Scripture as an amber-edged glass slab set in
/// Cormorant Garamond. The Mac original is pure SwiftUI, so this is that file
/// with phone-sized metrics.
...
private struct ListView: View {
    ...
    private func row(marker: String, text: String) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: Spacing.sm) {
            Text(marker)
                .font(.system(size: 15, weight: ordered ? .medium : .regular))
...
enum MarkdownInline {
    struct Style {
        ...
        static func body(_ theme: SureWordColors, size: CGFloat = 15) -> Style {
            Style(
