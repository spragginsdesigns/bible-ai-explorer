...
/// The iOS run of the macOS `NotesStoreTests` suite — the store and the
/// library model are shared, and the write-through cache behaviour they pin is
/// what the iOS list and editor both depend on.
///
/// `cacheURL: nil` keeps each store in memory, so these run without touching
/// the device's Application Support directory.
@Suite("Notes store (iOS)")
@MainActor
struct NotesStoreIOSTests {
...
/// The iOS list's filtering and sorting — pinned first, then the chosen order.
@Suite("Notes library sorting and filtering (iOS)")
@MainActor
struct NotesLibraryModelIOSTests {
