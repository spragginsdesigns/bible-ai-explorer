...
/// Manage screen for the memory feature — the native form of Android's
/// push-only `/memories` screen (`mobile/app/(app)/memories.tsx`): the
/// AI-written summary, adding, deleting and clearing. Reached from Settings →
/// Memory → Manage: a sheet on the Mac, a pushed route on iOS.
///
...
struct MemoriesView: View {
    ...
    /// Platform chrome: a fixed-size sheet with its own Done button on the Mac,
    /// a pushed route inside the tab's NavigationStack on iOS (where the nav
    /// bar's back button is the way out, and a fixed 520×600 frame would
    /// fight the layout).
    @ViewBuilder
