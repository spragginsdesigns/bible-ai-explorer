...
///
/// Shared by both Apple clients: the Mac pins it in a panel under the reader,
/// iOS presents it in the verse bottom sheet (which passes narrower
/// `skeletonWidths`).
struct VerseInsightView: View {
    ...
    let text: String
    let error: String?
    /// Definite (never greedy) widths for the three skeleton bars, at roughly
    /// the other clients' 100/92/64 proportions. The default fits the Mac's
    /// reader panel; the iOS sheet passes phone-width values.
    var skeletonWidths: [CGFloat] = [430, 396, 275]
    let onRetry: () -> Void
    ...
    /// Three ragged bars standing in for the answer's first lines.
    ///
    ...
    private var skeleton: some View {
        VStack(alignment: .leading, spacing: Spacing.sm) {
            ForEach(Array(skeletonWidths.enumerated()), id: \.offset) { index, width in
                SkeletonBar(width: width, delay: 0.18 * Double(index))
            }
        }
