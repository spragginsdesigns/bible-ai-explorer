...
- **Android releases go to the Play INTERNAL testing track, never closed testing** (Austin, 2026-09-20). `internal` is the only track that skips Play review, so a build reaches the phone in minutes; closed testing (`alpha`) queues behind review and has repeatedly left finished work invisible for hours. `push-phone.sh` defaults to `internal` and refuses any other track unless `SUREWORD_ALLOW_SLOW_TRACK=1` is set. The one legitimate reason to use closed testing is Play's own requirement of 12+ opted-in testers for 14 days before production access; that is an occasional deliberate act, done by promoting a build internal already has (`play-promote.mjs`), never a release step.
