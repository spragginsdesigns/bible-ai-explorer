...
function LearnAction({ onAdded, ...verse }: AddLearnProps) {
	...
	return <View style={styles.root}>
		{status === "saved" ? <View><Text accessibilityLiveRegion="polite" style={styles.message}>Added to Learn.</Text><Pressable accessibilityRole="button" onPress={() => router.push("/(app)/bible/learn")} style={styles.button}><Text style={styles.label}>Open Learn</Text></Pressable></View> :
			<Pressable accessibilityRole="button" accessibilityState={{ disabled: status === "saving" }} disabled={status === "saving"} onPress={() => void add()} style={[styles.button, status === "saving" && styles.disabled]}><Text style={styles.label}>{status === "saving" ? "Adding..." : "Learn this verse"}</Text></Pressable>}
