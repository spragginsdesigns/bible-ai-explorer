 ...
 * — the two are kept byte-for-byte in sync so both platforms repair the same
 * model-output quirks the same way. The logic is deliberately dependency-free
 * and Hermes-safe (no regex lookbehind). Keep the test vectors mirrored too
 * (`tests/assistant-markdown.test.mjs` on web,
