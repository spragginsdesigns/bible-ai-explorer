...
| Model + reasoning-effort picker (locked models point at Settings) | ✅ sparkles button in chat header (1.11.0) | ✅ picker on chat input | ✅ 1.5.0 sparkles toolbar button → popover | ✅ chat-header button → sheet | Served by `GET /api/ai/models`; sends `modelId`/`effort` in the chat body; last pick persists as the account default |
| Provider-grouped picker with live model lists (tap provider → every model its key unlocks, fetched from the provider) | ✅ accordion in picker sheet (1.12.0) | ✅ accordion in picker | ✅ 1.5.0 provider sections in popover | ✅ provider sections in picker sheet | Server lists models live per provider with the resolved key; curated registry is label source + outage fallback; effort only sent to models that support it |
